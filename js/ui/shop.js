...
import { CATEGORIES, itemsByCategory, monsterSVG } from "./monster.js";
...
export function renderShop(root, navigate) {
  ...
  root.innerHTML = `
    ...
    <main class="screen shop-screen">
      ...
      <p class="shop-subtitle">Earn coins by clearing quiz levels, then spend them here. 50 items across 7 categories.</p>
      ${sections}
    </main>
  `;
  ...
  root.querySelectorAll("[data-buy]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (gameState.purchase(id, cost)) {
        gameState.setAvatar({ [category]: id });
